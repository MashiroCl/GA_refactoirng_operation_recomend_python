...
class NameUnifier():
    def _append_csv(self, row):
        with open("./fixed_pull_request.csv_utils", "a") as fixed_csv:
            writer = csv.writer(fixed_csv)
            writer.writerow(row)
    def _write_csv(self, contents):
        with open("./fixed_pull_request_w.csv_utils", "w") as fixed_csv:
            writer = csv.writer(fixed_csv)
...
class PullRequestNameExtractor(NameExtractor):
    '''
    extract name set from pull request csv_utils
    '''
    def extract(self, source) -> set:
        result = set()
...
if __name__ == "__main__":
    ...
    csv_path = "/Users/leichen/ResearchAssistant/InteractiveRebase/data/atomix/MORCOoutput/csv/pullrequest_before_rename.csv"
    #
    NameUnifier().get_login_name_dict(csv_path, 0, 300)
